...
import { evaluateArchitecture, evaluateLedgerGrowth } from "./check-boundaries.mjs";
...
describe("clean architecture boundary gate", () => {
  it("rejects an application dependency on its Postgres adapter, including package aliases", () => {
    const root = fixture({ "backend/crates/orders/application/Cargo.toml": cargo('storage = { package = "mnt-orders-adapter-postgres", workspace = true }') });
  ...
  it("rejects direct persistence and lifecycle mutation anywhere in REST", () => {
    const root = fixture({ "backend/crates/orders/rest/src/helpers.rs": "async fn helper() { sqlx::query(\"UPDATE orders\").execute(&db).await; }" });
    assert.equal(evaluateArchitecture(root).failures[0].rule, "rest-delegation-boundary");
  });
  it("rejects generated client types outside adapters and generated API re-exports consumed by UI", () => {
    const root = fixture({
      "web/src/api/orderTypes.ts": 'import type { components } from "@maintenance/api-client-ts"; export type Order = components["schemas"]["Order"];',
  ...
  it("requires public.ts/index.ts for cross-feature imports even when the target is flat", () => {
    const root = fixture({ "web/src/features/orders/OrderPage.tsx": 'import "../inventory/private";', "web/src/features/inventory/private.ts": "export {};" });
