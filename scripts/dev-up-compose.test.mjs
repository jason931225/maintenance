...
const baseCompose = readFileSync(new URL("../ops/compose.yml", import.meta.url), "utf8");
const devUp = readFileSync(new URL("./dev-up.mjs", import.meta.url), "utf8");
const opsReadme = readFileSync(new URL("../ops/README.md", import.meta.url), "utf8");
const secretsDoc = readFileSync(new URL("../deploy/SECRETS.md", import.meta.url), "utf8");
const e2eDb = readFileSync(new URL("../e2e/harness/db.sh", import.meta.url), "utf8");
...
test("Compose migrates as mnt_app and serves as mnt_rt without owner/admin credentials", () => {
  ...
  assert.match(appBlock, /LEAVE_COMMAND_DATABASE_URL: postgresql:\/\/mnt_leave_cmd:/);
  assert.match(appBlock, /ONTOLOGY_COMMAND_DATABASE_URL: postgresql:\/\/mnt_ontology_cmd:/);
  assert.match(baseCompose, /x-app-env:[\s\S]*?DATABASE_URL: postgresql:\/\/mnt_rt:/);
  assert.doesNotMatch(workerBlock, /(?:LEAVE|ONTOLOGY)_COMMAND_DATABASE_URL/);
  assert.doesNotMatch(`${appBlock}\n${workerBlock}`, /mnt_app:|MNT_POSTGRES_ADMIN/);
  assert.match(baseCompose, /POSTGRES_USER: \$\{MNT_POSTGRES_ADMIN_USER:-mnt_cluster_admin\}/);
  assert.match(baseCompose, /migrate:[\s\S]*?DATABASE_URL: postgresql:\/\/mnt_app:/);
  assert.match(baseCompose, /migrate:[\s\S]*?postgres-topology:[\s\S]*?service_completed_successfully/);
  assert.match(baseCompose, /MNT_LEAVE_COMMAND_POSTGRES_PASSWORD:\s+\$\{[^}]+:\?/);
  assert.match(baseCompose, /MNT_ONTOLOGY_COMMAND_POSTGRES_PASSWORD:\s+\$\{[^}]+:\?/);
...
test("fresh and existing databases reconcile the exact hardened six-role topology", () => {
  for (const role of ["mnt_app", "mnt_rt", "mnt_leave_cmd", "mnt_ontology_cmd"]) {
  ...
  assert.match(commandRoleInit, /CREATE ROLE mnt_app LOGIN NOSUPERUSER BYPASSRLS INHERIT/);
  assert.match(commandRoleInit, /ALTER ROLE mnt_app LOGIN NOSUPERUSER BYPASSRLS INHERIT/);
  assert.match(commandRoleInit, /NOSUPERUSER NOBYPASSRLS NOINHERIT NOCREATEDB NOCREATEROLE/);
  ...
  assert.match(commandRoleInit, /topology\.legacy_default_acl_preflight_noncanonical/);
  assert.match(commandRoleInit, /topology\.legacy_default_acl_noncanonical/);
  assert.match(commandRoleInit, /ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA public REVOKE/);
...
test("quickstart supplies all five distinct login passwords and Compose accepts it", (t) => {
  for (const variable of [
    ...
    assert.match(opsReadme, new RegExp(`export ${variable}=\"\\$\\(openssl rand -hex 32\\)\"`));
  }
  ...
  const compose = candidates.find(([bin, prefix]) =>
  ...
  const result = spawnSync(bin, [...prefix, "-f", "ops/compose.yml", "config", "--quiet"], {
