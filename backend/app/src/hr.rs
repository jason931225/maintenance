...
pub const HR_LEAVE_REQUESTS_PATH: &str = "/api/v1/hr/leave-requests";
pub const HR_MY_LEAVE_REQUESTS_PATH: &str = "/api/v1/hr/leave-requests/me";
pub const HR_LEAVE_REQUEST_DECIDE_PATH_TEMPLATE: &str = "/api/v1/hr/leave-requests/{id}/decide";
pub const HR_LEAVE_PROMOTIONS_PATH: &str = "/api/v1/hr/leave-promotions";
pub const HR_LEAVE_PROMOTION_SEND_PATH_TEMPLATE: &str = "/api/v1/hr/leave-promotions/{id}/send";
pub const HR_ROUTE_PATHS: &[&str] = &[
    ...
    HR_MY_LEAVE_REQUESTS_PATH,
    HR_LEAVE_REQUEST_DECIDE_PATH_TEMPLATE,
    HR_LEAVE_PROMOTIONS_PATH,
    HR_LEAVE_PROMOTION_SEND_PATH_TEMPLATE,
];
...
pub fn router(state: HrState) -> Router {
    ...
    let router = Router::new()
        ...
        .route(
            HR_LEAVE_REQUESTS_PATH,
            get(list_team_leave_requests).post(create_leave_request),
        )
        .route(HR_MY_LEAVE_REQUESTS_PATH, get(list_my_leave_requests))
        .route(
            HR_LEAVE_REQUEST_DECIDE_PATH_TEMPLATE,
        ...
        .route(
            HR_LEAVE_PROMOTIONS_PATH,
            get(list_leave_promotions).post(create_leave_promotion),
        )
        .route(
            HR_LEAVE_PROMOTION_SEND_PATH_TEMPLATE,
            post(send_leave_promotion),
        )
        .with_state(state);
...
// ---------------------------------------------------------------------------
// HR leave requests + 연차 사용 촉진 (annual-leave usage promotion).
//
// Leave requests are owner-authored (self-service, no capability gate beyond a
// linked employee record) and decided by the branch HR/manager tier
// (EmployeeDirectoryManage). The decide step is separation-of-duties: the
// approver must differ from the requester — enforced here in code AND by the
// leave_requests_decider_distinct DB CHECK. Promotion rounds fan out to a
// snapshot set of target employees; each target's 수령확인 (receipt) state lives
// on leave_promotion_targets for the existing document/inbox flow to confirm.
// ---------------------------------------------------------------------------
...
/// FSM guard: a leave request can only be decided while SUBMITTED.
fn ensure_leave_request_decidable(status: &str) -> Result<(), HrError> {
    if status == "SUBMITTED" {
        Ok(())
    } else {
        Err(HrError::from_kernel(KernelError::invalid_transition(
            "only a submitted leave request can be decided",
        )))
    }
}

#[derive(Debug, Deserialize)]
struct CreateLeaveRequestRequest {
    leave_type: String,
    reason: String,
    start_date: String,
    end_date: String,
...
#[derive(Debug, Deserialize)]
struct DecideLeaveRequestRequest {
    /// "APPROVE" or "REJECT".
    decision: String,
    note: Option<String>,
}
...
struct MyLeaveOverview {
    ...
    requests: Vec<LeaveRequestResponse>,
}

#[derive(Debug, Serialize)]
struct MyLeaveBalance {
    employee_id: Uuid,
    #[serde(skip_serializing_if = "Option::is_none")]
    leave_accrued: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
#[derive(Debug, Serialize)]
struct LeaveRequestPage {
    items: Vec<LeaveRequestResponse>,
    limit: i64,
    offset: i64,
}

fn leave_request_from_row(row: &sqlx::postgres::PgRow) -> Result<LeaveRequestResponse, HrError> {
...
const LEAVE_REQUEST_SELECT: &str = r#"
    SELECT lr.id, lr.employee_id, e.name AS employee_name, lr.branch_id,
           lr.leave_type, lr.reason,
           lr.start_date::TEXT AS start_date, lr.end_date::TEXT AS end_date,
           lr.note, lr.status, lr.requested_by, lr.requested_at,
           lr.decided_by, lr.decided_at, lr.decision_note
    FROM leave_requests lr
    LEFT JOIN employees e ON e.id = lr.employee_id AND e.org_id = lr.org_id
"#;

async fn load_leave_request(
    tx: &mut sqlx::Transaction<'_, Postgres>,
    org_uuid: Uuid,
    request_id: Uuid,
) -> Result<LeaveRequestResponse, HrError> {
    let mut builder = QueryBuilder::<Postgres>::new(LEAVE_REQUEST_SELECT);
    builder
        .push(" WHERE lr.org_id = ")
        .push_bind(org_uuid)
        .push(" AND lr.id = ")
    ...
    let row = builder
        ...
        .fetch_optional(tx.as_mut())
        .await?
        .ok_or_else(|| HrError::from_kernel(KernelError::not_found("leave request not found")))?;
    leave_request_from_row(&row)
}
...
async fn create_leave_request(
...
) -> Result<Json<LeaveRequestResponse>, HrError> {
    let leave_type = normalize_leave_enum(&body.leave_type, LEAVE_TYPES, "leave_type")?;
    let reason = normalize_leave_enum(&body.reason, LEAVE_REASONS, "reason")?;
    let start_date = normalize_date_text(&body.start_date)?;
    let end_date = normalize_date_text(&body.end_date)?;
    ...
    let request = with_audits::<_, _, HrError>(&state.pool, org, move |tx| {
        Box::pin(async move {
            // Owner self-service: the caller must have a linked employee record;
            // the request is always filed for that employee (never an arbitrary id).
            let linked = load_linked_employee_for_user(tx, org, actor, false).await?;
            let branch_id =
                resolve_exit_case_branch(tx, org_uuid, linked.employee_id, None, None).await?;

            let request_id: Uuid = sqlx::query_scalar(
                r#"
                INSERT INTO leave_requests (
                    org_id, employee_id, branch_id, leave_type, reason,
                    start_date, end_date, note, status, requested_by
                )
                VALUES ($1, $2, $3, $4, $5, $6::DATE, $7::DATE, $8, 'SUBMITTED', $9)
                RETURNING id
                "#,
            )
            .bind(org_uuid)
            ...
            let request = load_leave_request(tx, org_uuid, request_id).await?;
            let event = AuditEvent::new(
                Some(actor),
                AuditAction::new("leave.request.create").map_err(HrError::from_kernel)?,
                "leave_request",
...
async fn list_my_leave_requests(
    State(state): State<HrState>,
    Extension(principal): Extension<Principal>,
) -> Result<Json<MyLeaveOverview>, HrError> {
    record_hr_read("leave_requests_self");
    let org = principal.org_id;
    let org_uuid = *org.as_uuid();
    let actor = principal.user_id;

    let overview = with_org_conn::<_, _, HrError>(&state.pool, org, move |tx| {
        Box::pin(async move {
            let linked = load_linked_employee_for_user(tx, org, actor, false).await?;
            ...
            .fetch_one(tx.as_mut())
            .await?;
            let balance = MyLeaveBalance {
                employee_id: balance_row.try_get("employee_id")?,
                leave_accrued: balance_row.try_get("leave_accrued")?,
                leave_used: balance_row.try_get("leave_used")?,
                leave_remaining: balance_row.try_get("leave_remaining")?,
            };
            ...
            builder
                .push(" WHERE lr.org_id = ")
                .push_bind(org_uuid)
                .push(" AND lr.employee_id = ")
                .push_bind(linked.employee_id)
                .push(" ORDER BY lr.requested_at DESC, lr.id DESC LIMIT ")
                .push_bind(MAX_LIMIT);
            let rows = builder.build().fetch_all(tx.as_mut()).await?;
            let requests = rows
                .iter()
                .map(leave_request_from_row)
                .collect::<Result<Vec<_>, _>>()?;

            Ok(MyLeaveOverview { balance, requests })
        })
    })
    .await?;
...
async fn list_team_leave_requests(
    State(state): State<HrState>,
    Extension(principal): Extension<Principal>,
    Query(query): Query<LeaveQueueQuery>,
) -> Result<Json<LeaveRequestPage>, HrError> {
    authorize_hr_scoped(&principal, Feature::EmployeeDirectoryManage)?;
    record_hr_read("leave_requests_team");
    let org = principal.org_id;
    let org_uuid = *org.as_uuid();
    let limit = query.limit.unwrap_or(DEFAULT_LIMIT).clamp(1, MAX_LIMIT);
    let offset = query.offset.unwrap_or(0).max(0);

    // Default queue view = pending (SUBMITTED). An explicit `status=all` widens it.
    let status_filter = match query.status.as_deref().map(str::trim) {
        None | Some("") => Some("SUBMITTED".to_owned()),
        Some("all") | Some("ALL") => None,
        Some(other) => {
            let normalized = other.to_ascii_uppercase();
            if ["DRAFT", "SUBMITTED", "APPROVED", "REJECTED"].contains(&normalized.as_str()) {
                Some(normalized)
            } else {
                return Err(HrError::validation(
                    "status must be one of: all, DRAFT, SUBMITTED, APPROVED, REJECTED",
                ));
            }
        }
    };

    // Team scoping: an org-wide (All) lead sees every request; a branch-scoped
    // lead sees only requests attributed to a branch it holds.
    let branch_filter: Option<Vec<Uuid>> = match &principal.branch_scope {
    ...
    let page = with_org_conn::<_, _, HrError>(&state.pool, org, move |tx| {
        Box::pin(async move {
            let mut builder = QueryBuilder::<Postgres>::new(LEAVE_REQUEST_SELECT);
            builder.push(" WHERE lr.org_id = ").push_bind(org_uuid);
            if let Some(status) = &status_filter {
                builder.push(" AND lr.status = ").push_bind(status.clone());
            }
            if let Some(branches) = &branch_filter {
                builder
                    .push(" AND lr.branch_id = ANY(")
                    .push_bind(branches.clone())
                    .push(")");
            }
            builder
                .push(" ORDER BY lr.requested_at DESC, lr.id DESC LIMIT ")
...
async fn decide_leave_request(
    ...
    Json(body): Json<DecideLeaveRequestRequest>,
) -> Result<Json<LeaveRequestResponse>, HrError> {
    let decision = body.decision.trim().to_ascii_uppercase();
    let next_status = match decision.as_str() {
        "APPROVE" => "APPROVED",
        "REJECT" => "REJECTED",
        _ => return Err(HrError::validation("decision must be APPROVE or REJECT")),
    };
    let note = normalize_optional_limited_text(body.note, 1000, "note")?;
    let org = principal.org_id;
    let org_uuid = *org.as_uuid();
    ...
    let event = AuditEvent::new(
        Some(actor),
        AuditAction::new("leave.request.decide").map_err(HrError::from_kernel)?,
        "leave_request",
        request_id.to_string(),
        TraceContext::generate(),
        OffsetDateTime::now_utc(),
    ...
    .with_org(org)
    .with_snapshots(None, Some(json!({ "decision": next_status })));

    let request = with_audit::<_, _, HrError>(&state.pool, event, |tx| {
        Box::pin(async move {
            let current = load_leave_request(tx, org_uuid, request_id).await?;
            authorize_hr_scoped_write(
                &principal,
                Feature::EmployeeDirectoryManage,
                current.branch_id,
            )?;
            ensure_leave_request_decidable(&current.status)?;
            // Separation of duties: the decider must differ from the requester.
            // The DB CHECK backstops this, but return a clean 403 first.
            if current.requested_by == *actor.as_uuid() {
                return Err(HrError::from_kernel(KernelError::forbidden(
                    "a leave request must be decided by a different user than the requester",
                )));
            }
            ...
            load_leave_request(tx, org_uuid, request_id).await
        })
    })
    .await?;
...
// --- 연차 사용 촉진 (annual-leave usage promotion) rounds -------------------

#[derive(Debug, Deserialize)]
struct CreateLeavePromotionRequest {
    round_no: i16,
    reference_year: i32,
    /// Employees whose remaining annual-leave days are at or above this
    /// threshold become promotion targets. 0 targets everyone with any balance.
    min_remaining_days: Option<f64>,
    note: Option<String>,
}

#[derive(Debug, Serialize)]
struct LeavePromotionResponse {
    id: Uuid,
    round_no: i16,
    reference_year: i32,
...
async fn load_leave_promotion(
    tx: &mut sqlx::Transaction<'_, Postgres>,
    org_uuid: Uuid,
    round_id: Uuid,
) -> Result<LeavePromotionResponse, HrError> {
    let row = sqlx::query(
        r#"
        SELECT r.id, r.round_no, r.reference_year, r.threshold_days::TEXT AS threshold_days,
               r.status, r.note, r.created_by, r.sent_at,
               COUNT(t.id) AS target_count,
               COUNT(t.id) FILTER (WHERE t.receipt_status = 'CONFIRMED') AS confirmed_count
        FROM leave_promotion_rounds r
        LEFT JOIN leave_promotion_targets t ON t.round_id = r.id AND t.org_id = r.org_id
    ...
    .fetch_optional(tx.as_mut())
    .await?
    .ok_or_else(|| HrError::from_kernel(KernelError::not_found("promotion round not found")))?;
    leave_promotion_from_row(&row)
}

fn leave_promotion_from_row(
    row: &sqlx::postgres::PgRow,
) -> Result<LeavePromotionResponse, HrError> {
    Ok(LeavePromotionResponse {
        id: row.try_get("id")?,
        round_no: row.try_get("round_no")?,
        reference_year: row.try_get("reference_year")?,
        threshold_days: row.try_get("threshold_days")?,
        status: row.try_get("status")?,
        note: row.try_get("note")?,
        created_by: row.try_get("created_by")?,
...
async fn create_leave_promotion(
    State(state): State<HrState>,
    Extension(principal): Extension<Principal>,
    Json(body): Json<CreateLeavePromotionRequest>,
) -> Result<Json<LeavePromotionResponse>, HrError> {
    authorize_hr_org_wide(&principal, Feature::EmployeeDirectoryManage)?;
    if body.round_no != 1 && body.round_no != 2 {
        return Err(HrError::validation("round_no must be 1 or 2"));
    }
    if !(2000..=2100).contains(&body.reference_year) {
        return Err(HrError::validation("reference_year is out of range"));
    }
    let threshold = body.min_remaining_days.unwrap_or(0.0);
    if !threshold.is_finite() || threshold < 0.0 {
        return Err(HrError::validation(
            "min_remaining_days must be a non-negative number",
        ));
    }
    let note = normalize_optional_limited_text(body.note, 1000, "note")?;
    let org = principal.org_id;
    let org_uuid = *org.as_uuid();
    let actor = principal.user_id;
    let round_no = body.round_no;
    let reference_year = body.reference_year;

    let round = with_audits::<_, _, HrError>(&state.pool, org, move |tx| {
        Box::pin(async move {
            let round_id: Uuid = sqlx::query_scalar(
                r#"
                INSERT INTO leave_promotion_rounds (
                    org_id, round_no, reference_year, threshold_days, status, note, created_by
                )
                VALUES ($1, $2, $3, $4::numeric, 'DRAFT', $5, $6)
                RETURNING id
                "#,
            )
            .bind(org_uuid)
            ...
            .map_err(|err| match err {
                sqlx::Error::Database(db) if db.code().is_some_and(|code| code == "23505") => {
                    HrError::from_kernel(KernelError::conflict(
                        "a promotion round already exists for this year/round",
                    ))
                }
                other => other.into(),
            })?;

            // Targets = active (non-exited) employees whose remaining annual leave
            // is at or above the threshold, snapshotted at round creation.
            sqlx::query(
                r#"
                INSERT INTO leave_promotion_targets (org_id, round_id, employee_id, remaining_days_snapshot)
                SELECT $1, $2, e.id, COALESCE(e.leave_remaining, 0)
                FROM employees e
                WHERE e.org_id = $1
                  AND e.exit_date IS NULL
                  AND COALESCE(e.leave_remaining, 0) >= $3::numeric
                "#,
            )
            .bind(org_uuid)
            .bind(round_id)
            ...
            let round = load_leave_promotion(tx, org_uuid, round_id).await?;
            let event = AuditEvent::new(
                Some(actor),
                AuditAction::new("leave.promotion.create").map_err(HrError::from_kernel)?,
                "leave_promotion_round",
                round_id.to_string(),
                TraceContext::generate(),
                OffsetDateTime::now_utc(),
            )
            ...
            .with_snapshots(
                ...
                Some(json!({
                    ...
                    "reference_year": round.reference_year,
                    "threshold_days": round.threshold_days,
...
async fn list_leave_promotions(
    State(state): State<HrState>,
    Extension(principal): Extension<Principal>,
) -> Result<Json<Vec<LeavePromotionResponse>>, HrError> {
    authorize_hr_org_wide(&principal, Feature::EmployeeDirectoryManage)?;
    record_hr_read("leave_promotions");
    let org = principal.org_id;
    let org_uuid = *org.as_uuid();

    let rounds = with_org_conn::<_, _, HrError>(&state.pool, org, move |tx| {
        Box::pin(async move {
            let rows = sqlx::query(
                r#"
                SELECT r.id, r.round_no, r.reference_year, r.threshold_days::TEXT AS threshold_days,
                ...
                WHERE r.org_id = $1
                GROUP BY r.id, r.org_id
                ORDER BY r.reference_year DESC, r.round_no ASC, r.created_at DESC
                "#,
            )
            .bind(org_uuid)
            .fetch_all(tx.as_mut())
...
async fn send_leave_promotion(
    ...
    let event = AuditEvent::new(
        Some(actor),
        AuditAction::new("leave.promotion.send").map_err(HrError::from_kernel)?,
        "leave_promotion_round",
        round_id.to_string(),
        TraceContext::generate(),
        OffsetDateTime::now_utc(),
    ...
    let round = with_audit::<_, _, HrError>(&state.pool, event, |tx| {
        Box::pin(async move {
            let current = load_leave_promotion(tx, org_uuid, round_id).await?;
            if current.status != "DRAFT" {
                return Err(HrError::from_kernel(KernelError::invalid_transition(
                    "only a draft promotion round can be sent",
                )));
            }
            if current.target_count == 0 {
                return Err(HrError::validation(
                    "cannot send a promotion round with no target employees",
                ));
            }
            sqlx::query(
                r#"
                UPDATE leave_promotion_rounds
            ...
            .execute(tx.as_mut())
            .await?;
            load_leave_promotion(tx, org_uuid, round_id).await
        })
    })
    .await?;
...
";
    ...
    // ---------------------------------------------------------------------
    // HR leave requests + 연차 사용 촉진 promotion.
    // ---------------------------------------------------------------------

    #[test]
    fn leave_enum_normalizes_and_fails_closed() {
        assert_eq!(
            normalize_leave_enum("annual", LEAVE_TYPES, "leave_type").unwrap(),
            "ANNUAL"
        );
        assert_eq!(
            normalize_leave_enum("  Medical ", LEAVE_REASONS, "reason").unwrap(),
            "MEDICAL"
        );
        let err = normalize_leave_enum("VACATION", LEAVE_TYPES, "leave_type").unwrap_err();
        assert_eq!(err.status, StatusCode::UNPROCESSABLE_ENTITY);
        assert!(normalize_leave_enum("", LEAVE_REASONS, "reason").is_err());
    }

    #[test]
    fn leave_decide_fsm_only_from_submitted() {
        ensure_leave_request_decidable("SUBMITTED").expect("submitted must be decidable");
        for terminal in ["DRAFT", "APPROVED", "REJECTED"] {
            let err = ensure_leave_request_decidable(terminal).unwrap_err();
            assert_eq!(
                err.code, "invalid_transition",
                "{terminal} must not be decidable"
            );
        }
    }

    /// Seed org + region + branch + one employee (with a leave balance) linked to
    /// a MEMBER user. Seeding bypasses RLS; assertions run as `mnt_rt`.
    async fn seed_leave_base(
        pool: &sqlx::PgPool,
        remaining: i32,
        ...
        sqlx::query("INSERT INTO organizations (id, slug, name) VALUES ($1, $2, $3)")
            .bind(org_id)
            .bind(format!("leave-{}", &org_id.to_string()[..8]))
            .bind("Leave Workflow Test")
            .execute(pool)
            .await
            .map_err(|err| format!("seed organization failed: {err}"))?;
        sqlx::query("INSERT INTO regions (id, name, org_id) VALUES ($1, $2, $3)")
        ...
        sqlx::query("INSERT INTO branches (id, region_id, name, org_id) VALUES ($1, $2, $3, $4)")
            ...
            .await
            .map_err(|err| format!("seed branch failed: {err}"))?;
        seed_leave_employee(pool, org_id, employee_id, "E-001", remaining, false).await?;
        sqlx::query(
            "INSERT INTO users (id, display_name, roles, is_active, org_id, employee_id) VALUES ($1, $2, ARRAY['MEMBER']::TEXT[], true, $3, $4)",
        )
        .bind(user_id)
        .bind("휴가 신청자")
        .bind(org_id)
    ...
    async fn seed_leave_employee(
        ...
        let exit_date = if exited { Some("2026-05-31") } else { None };
        sqlx::query(
            r#"
            INSERT INTO employees (
                id, org_id, company, name, employee_number, hire_date, exit_date,
                leave_accrued, leave_used, leave_remaining,
                source_filename, source_sheet, source_row, source_key, raw_row, source_metadata
            )
            VALUES ($1, $2, '테스트', '홍길동', $3, '2020-01-01', $4::DATE,
                    15, 3, $5::numeric,
                    'employees.xlsx', '직원', 2, $6, '{}', '{}')
            "#,
        )
        .bind(employee_id)
        .bind(org_id)
        ...
        .bind(exit_date)
        .bind(remaining)
        .bind(format!("employee-{employee_number}"))
        .execute(pool)
        .await
        .map_err(|err| format!("seed employee failed: {err}"))?;
        Ok(())
    ...
    async fn insert_submitted_leave_request(
        pool: &sqlx::PgPool,
        org_id: Uuid,
        branch_id: Uuid,
        employee_id: Uuid,
        ...
        sqlx::query(
            r#"
            INSERT INTO leave_requests (
                id, org_id, employee_id, branch_id, leave_type, reason,
                start_date, end_date, status, requested_by
            )
            VALUES ($1, $2, $3, $4, 'ANNUAL', 'PERSONAL', '2026-08-01', '2026-08-02', 'SUBMITTED', $5)
            "#,
        )
        .bind(id)
        .bind(org_id)
        ...
        .execute(pool)
        .await
        .map_err(|err| format!("seed leave request failed: {err}"))?;
        Ok(id)
    }

    /// Self-approval is rejected by the `leave_requests_decider_distinct` CHECK
    /// AND a distinct decider succeeds — both exercised as the real `mnt_rt`
    /// runtime role (proves mnt_rt can write the table under RLS).
    #[sqlx::test(migrations = "../crates/platform/db/migrations")]
    async fn leave_self_approval_rejected_distinct_allowed_as_mnt_rt(
        pool: sqlx::PgPool,
    ) -> Result<(), String> {
        let (org_id, branch_id, employee_id, requester) = seed_leave_base(&pool, 12).await?;
        let approver = seed_exit_confirmer(&pool, org_id, "ADMIN").await?;

        // INSERT the request as mnt_rt (proves the runtime role writes under RLS).
        let mut tx = armed_tx(&pool, org_id).await?;
        ...
        .execute(tx.as_mut())
        .await
        .map_err(|err| format!("mnt_rt insert failed: {err}"))?;
        tx.commit()
            .await
            .map_err(|err| format!("commit insert failed: {err}"))?;

        // (a) self-approval violates the distinct-decider CHECK.
        let mut tx = armed_tx(&pool, org_id).await?;
        let self_approve = sqlx::query(
            "UPDATE leave_requests SET status = 'APPROVED', decided_by = $3, decided_at = now() WHERE org_id = $1 AND id = $2",
        )
        .bind(org_id)
        .bind(request_id)
        .bind(requester)
        ...
        assert!(
            self_approve.is_err(),
            "self-approval must be rejected by leave_requests_decider_distinct"
        );
        tx.rollback()
            .await
            .map_err(|err| format!("rollback failed: {err}"))?;

        // (b) a distinct decider is accepted, as mnt_rt.
        let mut tx = armed_tx(&pool, org_id).await?;
        sqlx::query(
            "UPDATE leave_requests SET status = 'APPROVED', decided_by = $3, decided_at = now() WHERE org_id = $1 AND id = $2",
        )
        ...
        .execute(tx.as_mut())
        .await
        .map_err(|err| format!("distinct-decider approve failed: {err}"))?;
        let (status, decided_by): (String, Option<Uuid>) = sqlx::query_as(
            "SELECT status, decided_by FROM leave_requests WHERE org_id = $1 AND id = $2",
        )
        .bind(org_id)
        .bind(request_id)
        .fetch_one(tx.as_mut())
        ...
        assert_eq!(decided_by, Some(approver));
        tx.rollback()
            .await
            .map_err(|err| format!("rollback failed: {err}"))?;
        Ok(())
    ...
    /// leave_requests are invisible AND un-writable across orgs as `mnt_rt`.
    #[sqlx::test(migrations = "../crates/platform/db/migrations")]
    async fn leave_requests_isolate_tenants_as_mnt_rt(pool: sqlx::PgPool) -> Result<(), String> {
        let (org_a, branch_a, emp_a, user_a) = seed_leave_base(&pool, 10).await?;
        let (org_b, branch_b, emp_b, user_b) = seed_leave_base(&pool, 10).await?;
        insert_submitted_leave_request(&pool, org_a, branch_a, emp_a, user_a).await?;
        insert_submitted_leave_request(&pool, org_b, branch_b, emp_b, user_b).await?;

        let mut tx = armed_tx(&pool, org_a).await?;
        let visible: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM leave_requests")
            .fetch_one(tx.as_mut())
            .await
            .map_err(|err| format!("count as mnt_rt failed: {err}"))?;
        assert_eq!(
            visible, 1,
            "org A (mnt_rt) must see only its own leave request"
        );

        // Writing an org B row while armed to org A is refused by RLS WITH CHECK.
        let cross = sqlx::query(
            ...
            VALUES ($1, $2, $3, 'ANNUAL', 'PERSONAL', '2026-08-01', '2026-08-02', 'SUBMITTED', $4)
            "#,
        )
        .bind(org_b)
        .bind(emp_b)
        ...
        assert!(
            cross.is_err(),
            "org A (mnt_rt) must not INSERT an org B leave request"
        );
        tx.rollback()
            .await
            .map_err(|err| format!("rollback failed: {err}"))?;
    ...
    /// Owner scoping: the employee-filtered read (the /me path's query) returns
    /// only the caller's own requests, as `mnt_rt`.
    #[sqlx::test(migrations = "../crates/platform/db/migrations")]
    async fn owner_leave_query_excludes_other_employees_as_mnt_rt(
        pool: sqlx::PgPool,
    ) -> Result<(), String> {
        let (org_id, branch_id, emp_a, user_a) = seed_leave_base(&pool, 10).await?;
        let emp_b = Uuid::new_v4();
        seed_leave_employee(&pool, org_id, emp_b, "E-002", 10, false).await?;
        let user_b = seed_exit_confirmer(&pool, org_id, "MEMBER").await?;
        insert_submitted_leave_request(&pool, org_id, branch_id, emp_a, user_a).await?;
        insert_submitted_leave_request(&pool, org_id, branch_id, emp_b, user_b).await?;

        let mut tx = armed_tx(&pool, org_id).await?;
        let mine: Vec<Uuid> = sqlx::query_scalar(
            "SELECT employee_id FROM leave_requests WHERE org_id = $1 AND employee_id = $2",
        )
        .bind(org_id)
        .bind(emp_a)
        .fetch_all(tx.as_mut())
        .await
        .map_err(|err| format!("owner query failed: {err}"))?;
        assert_eq!(mine, vec![emp_a], "owner must see only their own request");
        tx.rollback()
            .await
            .map_err(|err| format!("rollback failed: {err}"))?;
        Ok(())
    ...
    /// Promotion round targets are the active employees at/above the threshold —
    /// computed by the real INSERT...SELECT and read back via `load_leave_promotion`
    /// as `mnt_rt`. Exited and below-threshold employees are excluded.
    #[sqlx::test(migrations = "../crates/platform/db/migrations")]
    async fn promotion_targets_computed_as_mnt_rt(pool: sqlx::PgPool) -> Result<(), String> {
        // The base employee (remaining 12, active) qualifies; the rest do not.
        let (org_id, _, _, _) = seed_leave_base(&pool, 12).await?;
        let emp_low = Uuid::new_v4();
        seed_leave_employee(&pool, org_id, emp_low, "E-LOW", 2, false).await?;
        let emp_exited = Uuid::new_v4();
        seed_leave_employee(&pool, org_id, emp_exited, "E-EXIT", 20, true).await?;
        let creator = seed_exit_confirmer(&pool, org_id, "ADMIN").await?;
        ...
        sqlx::query(
            r#"
            INSERT INTO leave_promotion_rounds (
                id, org_id, round_no, reference_year, threshold_days, status, created_by
            )
            VALUES ($1, $2, 1, 2026, 10, 'DRAFT', $3)
            "#,
        )
        .bind(round_id)
        .bind(org_id)
        ...
        .execute(tx.as_mut())
        .await
        .map_err(|err| format!("insert round failed: {err}"))?;
        ...
        sqlx::query(
            ...
            WHERE e.org_id = $1
              AND e.exit_date IS NULL
              AND COALESCE(e.leave_remaining, 0) >= 10::numeric
            "#,
        )
        .bind(org_id)
        .bind(round_id)
        .execute(tx.as_mut())
        .await
        .map_err(|err| format!("compute targets failed: {err}"))?;

        let round = load_leave_promotion(&mut tx, org_id, round_id)
            .await
            .map_err(|err| format!("load promotion failed: {err:?}"))?;
        assert_eq!(
            round.target_count, 1,
            "only the active, at/above-threshold employee is a target"
        );
