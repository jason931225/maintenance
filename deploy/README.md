...
- **Ordering:** CNPG is wave 0; the wave-1 `mnt-db-topology` Sync hook blocks
  ...
  memberships read back exactly. The same gate verifies the four decoded login
  passwords are pairwise distinct and opens real direct sessions as `mnt_app`,
  `mnt_rt`, `mnt_leave_cmd`, and `mnt_ontology_cmd`; no admin-plus-`SET ROLE`
  alias is accepted. `mnt-migrate` is a wave-2 **Sync hook** and API/worker
  workloads are wave 3. A topology or migration failure blocks the serving
  rollout. PreSync is deliberately not used because it runs before the Cluster
  can be created on a fresh installation.
...
- **Rollout-wide connection budget:** the runtime pool is capped at six
  connections per API/worker process and each API command pool at two. During
  the worst-case blue/green plus worker surge this is 52 connections, leaving
  eight of PostgreSQL's configured 60 for migration, topology, and operator
  access. Raise these values only as one reviewed budget; per-process tuning in
  isolation can make an otherwise healthy rollout exhaust the database.
