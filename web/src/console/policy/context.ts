...
// local one defaults to allow when no provider wraps the tree (dev-safe) and is
// driven by a caller-supplied decider (JWT role hint) in LifecycleCard.
...
export const PolicyContext = createContext<PolicyDecider>(() => true);
