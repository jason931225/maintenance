...
import { computeDropdownPosition, type DropdownPlacement, type TriggerChar } from "./grammar";
...
/**
 * The single token-grammar composer (DESIGN §4.7-7): a textarea with a
 * candidate dropdown on `@`/`#`/`!` and a live chip preview. Only objects that
 * flow through a permission-scoped provider become resolvable chips — a
 * hand-typed code that never matched a permitted candidate stays inert plain
 * text (deny-by-omission), so `resolveObject` mirrors the server's
 * authorization, it is never the console's own check.
 */
