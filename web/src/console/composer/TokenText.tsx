...
/**
 * Renders stored token-grammar text (DESIGN §4.7-7): `@mention` styling and
 * `#object-link` / `!code-link` chips. Maps over the parser's DATA array
  ...
  return (
    <span style={wrapStyle} data-testid="token-text">
      {spans.map((span, index) => {
        ...
        // `@` resolves only to "person" here; `#`/`!` derive the kind from the
        // code prefix. (Org/team `@` mentions await their candidate provider.)
        ...
        return (
          <button
            ...
            aria-label={`${KIND_META[kind].label} ${code} ${label}`}
