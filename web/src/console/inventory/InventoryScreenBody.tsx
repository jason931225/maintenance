...
function InventoryOperations({ api, item }: { api: ConsoleApiClient; item: InventoryItem }) {
  ...
  async function receipt() { const milli = milliUnits(amount); if (milli == null) { setMessage("입고 수량은 0보다 큰 셋째 자리 단위여야 합니다."); return; } const payload = JSON.stringify([item.id, milli, sourceRef.trim()]); if (receiptAttempt.current?.payload !== payload) receiptAttempt.current = { payload, key: crypto.randomUUID() }; setBusy(true); try { await receiveInventoryItem(api, item.id, { quantity_received_milli: milli, source_ref: sourceRef.trim() || undefined, idempotency_key: receiptAttempt.current.key }); receiptAttempt.current = null; setShowReceipt(false); setAmount(""); setSourceRef(""); await load(); } catch (error) { setMessage(isAccessDenied(error) ? "입고 권한이 없습니다." : "입고가 저장되지 않았습니다. 같은 내용으로 다시 제출하면 중복을 방지합니다."); } finally { setBusy(false); } }
  async function openCount() { setBusy(true); try { setCount(await openCycleCount(api, item.branch_id, item.stock_location.id)); await load(); } catch (error) { setMessage(isAccessDenied(error) ? "실사 개설 권한이 없습니다." : "실사 개설에 실패했습니다."); } finally { setBusy(false); } }
  async function line() { if (!count) return; const milli = nonNegativeMilliUnits(counted); if (milli == null) { setMessage("실사 수량은 0 이상 정수/소수 셋째 자리로 입력하세요."); return; } if (milli !== count.lines.find((line) => line.item_id === item.id)?.system_quantity_milli && !reason) { setMessage("시스템 수량과 다른 실사는 차이 사유가 필요합니다."); return; } setBusy(true); try { setCount(await upsertCycleLine(api, count.count.id, { item_id: item.id, counted_quantity_milli: milli, reason: reason || undefined, note: memo.trim() || undefined })); } catch (error) { setMessage(isAccessDenied(error) ? "실사 라인을 변경할 권한이 없습니다." : "실사 라인이 저장되지 않았습니다. 최신 상태를 다시 확인하세요."); } finally { setBusy(false); } }
