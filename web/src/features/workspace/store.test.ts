...
const wo: PinnedObject = { kind: "workOrder", code: "WO-1", title: "작업 1", fields: [] };
...
describe("useWorkspaceStore", () => {
  it("hydrate replaces panels and flips the hydrated flag", () => {
    useWorkspaceStore.getState().hydrate([
      { id: "work-hub:WO-1", screen: "work-hub", object: wo, area: "right", mode: "pinned" },
    ]);
  ...
  it("pin / minimize / restore / close flow through the store", () => {
    ...
    store.minimize("work-hub:WO-1");
    expect(useWorkspaceStore.getState().panels[0].mode).toBe("minimized");
    store.restore("work-hub:WO-1");
    expect(useWorkspaceStore.getState().panels[0].mode).toBe("pinned");
    store.close("work-hub:WO-1");
  ...
  it("selectScreenPanels isolates each screen's panels", () => {
    ...
    expect(selectScreenPanels(panels, "attendance")[0].object.code).toBe("AT-1");
  });
  ...
  it("tracks transient drag preview state", () => {
    const store = useWorkspaceStore.getState();
    store.setDragging("work-hub:WO-1");
    store.setSnapPreview("right");
    expect(useWorkspaceStore.getState().draggingId).toBe("work-hub:WO-1");
